...
?>
	<form method="post" id="add" name="add" action="menu_tab_edit.php?save<?php echo $add; if ($menuset) echo '&amp;menuset='.$menuset; ?>" style="display: none">
		...
		if(is_array($result)) {
			...
			<tr id="visible_row">
				...
				<td>
					<label id="include_li_label" style="display: inline">
						<input name="include_li" type="checkbox" id="include_li" value="1" <?php if ($result['include_li'] == 1) { echo "checked='checked'"; } ?> style="display: inline" />
			...
			<tr id="span_row">
				<td>
					<input name="span" type="checkbox" id="span" value="1" <?php if ($result['span_id'] || $result['span_class']) { echo "checked='checked'"; } ?> style="display: inline" />
