...
<script language="javascript" type="text/javascript" >
	...
	function aspectChange() {
		...
		if (!aspectWidth) aspectWidth = aspectHeight;
		if (!aspectHeight) aspectHeight = aspectWidth;
		if (aspectHeight) {
				...
				<div style="text-align:left; float: left;">
					...
					<span style="line-height: 0em;"><br clear="all" /></span>
						<?php printf(gettext('width:%1$s %2$s height:%3$s %4$s clear %5$s'),
										...
										'<input type="text" id="aspect-ratio-height" name="aspect-ratio-height" value="" size="5" />',
										'<a href="javascript:clearAspect();" title="'.gettext('clear width and height fields').'" >',
										'</a>')?>
