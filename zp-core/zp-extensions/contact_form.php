...
function printContactForm($subject_override='') {
	...
	if(isset($_POST['sendmail'])) {
		...
		if (getOption('contactform_title') == "required" && empty($mailcontent['title'])) { $error[1] = gettext("a title"); }
		if (getOption('contactform_name') == "required" && empty($mailcontent['name'])) { $error[2] = gettext("a name"); }
